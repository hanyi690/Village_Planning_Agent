...
│   └── logging.py             # 日志配置 ⭐ 2024更新
│
├── services/                  # 业务逻辑层
│   └── rate_limiter.py        # 限流服务 ⭐ NEW
│
└── (规划引擎在 src/ 目录)
```
...
### 1. Pause 事件去重机制 (2025-02) ⭐⭐⭐ NEW

**问题修复**: 修复审查面板重复显示和批准失败问题

**根本原因**:
- 后端 Pause 事件没有去重机制，stream iterator 可能产生重复的 `pause_after_step=true` 事件
- 前端 Pause 事件处理没有去重检查，每次收到 pause 事件都添加审查消息
- 批准后状态清理不完整，导致状态冲突
...
2. **在添加 pause 事件前去重** (Lines 324-347):
```python
# 检查暂停状态（步进模式）
...
3. **批准后清理 pause 事件追踪** (Lines 711-730):
```python
if request.action == "approve":
    # ✅ 详细日志记录批准前的状态
    logger.info(f"批准请求 - 状态检查")
...
**关键函数调用关系**:
```
_execute_graph_in_background()  [Line ~200]
  ├─> _sessions[session_id].setdefault("sent_pause_events", set())  [Line 266]
  ...
  │    └─> return (跳过重复)
  └─> _format_sse_event()  [Line ~800]

review_action()  [Line 670]
  ├─> 检查 action == "approve"  [Line 711]
  ├─> logger.info() 批准前状态日志  [Lines 714-718]
  ├─> _stream_states[session_id] = "active"  [Line 721]
  ├─> initial_state["pause_after_step"] = False  [Line 727]
  ├─> 清除 sent_pause_events  [Line 733]
  └─> _resume_graph_execution()  [Line 748]
...
backend/api/planning.py
  ├─> src/orchestration/main_graph.py  # create_village_planning_graph()
  ├─> src.utils.output_manager.py      # create_output_manager()
  ├─> backend/services/rate_limiter.py # rate_limiter.is_allowed()
  └─> backend/utils/logging.py         # get_logger()
```

### 2. SSE 流式传输优化

**关键修复**: 修复了 SSE 连接无限循环问题

**实现细节**:
- 添加 `stream_paused` 事件通知前端关闭连接
- 实现 `_stream_states` 全局状态跟踪
- 批准后创建新的 SSE 连接继续执行
...
# 记录请求
logger.info("Processing planning request", extra={
    "session_id": session_id,
