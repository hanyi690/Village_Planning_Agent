...
| 维度级 doc_type 过滤 | `phases.yaml` → `prefer_doc_types` 按文档类型优选检索 |
| 分层差异化查询 | L1 侧重技术标准、L3 侧重规划编制+案例 |
| metadata 全链路回传 | doc_type/title/category/subcategory 贯穿 ChromaDB→retrieve→RetrievedChunk |
...
| `search()` | 向量搜索（支持 doc_type 过滤） | query, top_k, prefer_doc_types |
...
`generate_queries()` 根据维度所在层构建不同的 LLM Prompt，生成差异化查询：

```python
def generate_queries(cfg, state) -> List[str]:
    dim_name = cfg.name          # 维度中文名
    task_desc = cfg.rag_query    # 从 phases.yaml 读取
    layer = cfg.phase_id          # 1/2/3
    prefer_doc_types = cfg.prefer_doc_types  # 可选文档类型过滤
```

**Layer 1（现状分析，12 维度）**：
- `rag_query` 示例：`"村庄区位条件分析 对外交通规划要求 交通设施配置标准"`
- LLM Prompt 侧重：**技术方法和标准规范**
- 生成 4-6 条查询，侧重检索：技术规范、数据采集方法、评价指标
- `prefer_doc_types`：根据维度设为 `[standard, guide]` 或 `[policy, standard]`

**Layer 2（规划思路，4 维度）**：
- `rag_query` 均为空 → **RAG 被跳过**
- 原因：Layer 2 依赖 Layer 1 报告内容，RAG 外部知识在此层无必要
- `_run_rag()` 中守卫检查：`if not dim_rag_query: return ("", None)`

**Layer 3（详细规划，12 维度）**：
- `rag_query` 示例：`"村庄产业发展规划 农村产业布局 一二三产融合"`
- LLM Prompt 侧重：**规划编制标准、政策要求、相关案例**
- 生成 4-6 条查询，Prompt 中包含依赖维度摘要（作为上下文参考）
- `prefer_doc_types`：按需设 `[standard, case]` / `[policy, standard]` 等
- 特殊：`project_bank` 的 `rag_query` 为空，跳过 RAG

**Layer 3 Prompt 模板差异**：
```
Layer 1 Prompt:
  你是乡村规划数据检索专家。请为"{dim_name}"生成检索查询。
  该维度属于"现状分析"层，侧重收集技术方法和评估标准。
  
Layer 3 Prompt:
  你是乡村规划知识检索专家。请为"{dim_name}"生成检索查询。
  该维度属于"详细规划"层，侧重规划编制标准、政策要求和案例参考。
  参考以下已完成的维度分析结果：
...
文档从 `_doc_md` 导入到存入 `DimensionReport.knowledge_sources`，metadata 经过 5 个节点：

```
_doc_md 目录 ──→ MetadataExtractor ──→ chunk.metadata ──→ ChromaDB ──→ retrieve() ──→ RetrievedChunk ──→ knowledge_sources
```

各节点持有的字段（✅=存在，❌=缺失）：

| 字段 | chunk.metadata | ChromaDB | retrieve() | RetrievedChunk | knowledge_sources |
|------|:---:|:---:|:---:|:---:|:---:|
| source | ✅ | ✅ | ✅ | ✅ | ✅ |
| doc_type | ✅ | ✅ | ✅ | ✅ | ❌(旧数据) |
| title | ✅ | ✅ | ✅ | ✅ | ❌(旧数据) |
...
**说明**：
- `chunk.metadata` 有15+字段，在 `add_document()` 时由 `MetadataExtractor` 设置
- ChromaDB 最初只存7字段（白名单），2026-05-20 补充 `doc_type`/`title`/`category`/`subcategory`
- `retrieve()` 最初只返回6字段，2026-05-20 补充4字段
- `knowledge_sources` 在 `analysis.py` 通过 `asdict(rag_log)` 写入，**旧 session 数据不含新字段**，新 session 运行后自动生效
- `keywords` 在 ChromaDB 创建时被丢弃，`dimension_tags` 已删除

#### _doc_md 目录 → 文档类型映射

```
data/RAG_doc/_doc_md/
├── 01_专业教材/          → doc_type=textbook
├── 02_法律法规/
│   ├── 01_法律/           → doc_type=policy, subcategory=法律
│   ├── 02_地方性法规/     → doc_type=policy, subcategory=地方性法规
│   └── 03_行政法规/       → doc_type=policy, subcategory=行政法规
├── 03_政策文件/
│   ├── 01_国家层面/       → doc_type=policy, subcategory=国家层面
│   └── 02_地方层面/       → doc_type=policy, subcategory=地方层面
├── 04_技术规范/           → doc_type=standard
├── 05_上位规划/           → doc_type=planning
└── 06_相关案例/           → doc_type=case
```

总计 78 文档、15,275 chunks。完整映射见 `data/RAG_doc/_cache/source_metadata_map.json`。

### 3.7 维度级 doc_type 过滤

#### phases.yaml 配置

每个维度在 `phases.yaml` 中可设 `prefer_doc_types`，声明优先检索的文档类型：

```yaml
# 示例：上位规划维度 → 优先法规+上位规划
- key: superior_planning
  rag_query: "上位规划与政策导向分析"
  prefer_doc_types: [policy, planning]

# 示例：生态绿地维度 → 优先技术标准+指南
- key: ecological
  rag_query: "村庄绿地规划 生态保护用地 公园绿地设计"
  prefer_doc_types: [standard, guide]

# 示例：历史文化维度 → 优先法规+标准
- key: historical_culture
  rag_query: "历史文化保护规划 传统建筑保护"
  prefer_doc_types: [policy, standard]
```

配置加载后通过 `DimensionConfig.prefer_doc_types` 访问，在 `analysis.py:_run_rag()` 中传入检索。

#### 过滤机制

`vector_store.retrieve()` 使用 ChromaDB 的 `filter` 参数：

```python
def retrieve(self, query, k=5, doc_types=None, ...):
    chroma_filter = None
    if doc_types:
...
**完整渗透路径**：`phases.yaml` → `load_config()` → `DimensionConfig.prefer_doc_types` → `analysis.py:_run_rag()` → `RagService.search(prefer_doc_types=...)` → `vector_store.retrieve(doc_types=...)` → ChromaDB filter

#### 23 个维度的 prefer_doc_types 分布

| 维度 | prefer_doc_types |
|------|-----------------|
| location, traffic, natural_environment, infrastructure, ecological_green | `[standard, guide]` |
| socio_economic, land_use, public_services, architecture, historical_culture | `[policy, standard]` |
| villager_wishes | `[standard, guide]` |
| superior_planning | `[policy, planning]` |
| industry | `[policy, standard, case]` |
| spatial_structure, settlement_planning | `[standard, case]` |
| land_use_planning, disaster_prevention, heritage | `[policy, standard]` |
| traffic_planning, public_service | `[standard]` |
| infrastructure_planning, ecological | `[standard, guide]` |
| landscape | `[standard, guide, case]` |
| resource_endowment, planning_positioning, development_goals, planning_strategies, project_bank | （无 RAG，空 rag_query） |
...
**现象**：知识库有 12 部法律法规（《土地管理法》《城乡规划法》等），但 RAG 查询从未命中，第4条规划依据缺少法律条文。

**原因**：
- `generate_queries()` 的 LLM Prompt 侧重技术指标（如"人均建设用地指标"）
- 生成的查询词不包含法律名称精确匹配
- ChromaDB 语义搜索在 15K chunks 中偏向技术内容

**解决方案**：
- `phases.yaml` 中 `superior_planning` 已设 `prefer_doc_types: [policy, planning]`
- 新 session 中将优先检索法规+上位规划类文档
- 进一步优化：对 `superior_planning` 增加精确关键词查询（如包含 `中华人民共和国` 的文档名）
...
| 日期 | 版本 | 更新内容 |
|------|------|----------|
| 2026-05-20 | v8.0 | Metadata 全链路文档化（doc_type/title/category/subcategory）；维度级 doc_type 过滤；L1/L2/L3 分层查询差异详解；删除 dead code `dimension_tags`；ChromaDB 15275 embedding metadata 补注入 |
