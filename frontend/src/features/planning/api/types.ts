...
/**
 * GIS 数据类型 - 与后端 LAYER_TYPE_KEYWORDS 保持同步
 */
export type GISDataType =
  | 'boundary'
  | 'landuse'
  | 'road'
  | 'water'
 ...
 * GIS 数据类型名称映射
 */
export const GIS_DATA_TYPE_NAMES: Record<GISDataType, string> = {
  boundary: '行政边界',
  landuse: '土地利用',
  road: '道路网络',
  water: '水系',
 ...
 * GIS 上传元数据
 */
export interface GISUploadMetadata {
  fileName: string;
  fileSize: number;
  fileType: string;
  featureCount: number;
...
/**
 * Backend GIS upload metadata (snake_case)
 */
export interface BackendGISUploadMetadata {
  file_name: string;
  file_size: number;
  file_type: string;
...
/**
 * Backend GIS upload result (snake_case)
 */
export interface BackendGISUploadResult {
  success: boolean;
  village_name: string;
  data_type: string;
  geojson: GeoJSON.FeatureCollection;
  metadata: BackendGISUploadMetadata;
  source: string;
  error?: string;
}
...
export interface GISDataStatus {
  villageName: string;
  userUploaded: GISDataType[];
  cached: GISDataType[];
  missing: GISDataType[];
  metadata: Record<string, GISUploadMetadata>;
}

/**
 * Backend GIS data status (snake_case)
 */
export interface BackendGISDataStatus {
  village_name: string;
  user_uploaded: string[];
  cached: string[];
  missing: string[];
  metadata: Record<string, BackendGISUploadMetadata>;
}

/**
 * 支持的 GIS 格式信息
 */
export interface GISSupportedFormat {
  ext: string;
  type: string;
  description: string;
}
 ...
 * 支持的格式响应
 */
export interface GISSupportedFormatsResponse {
  supported: GISSupportedFormat[];
  unsupported: GISSupportedFormat[];
