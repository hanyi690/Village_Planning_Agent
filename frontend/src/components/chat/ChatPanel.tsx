...
import type { Message, FileMessage, ProgressMessage, ActionButton } from '@/types';
...
export default function ChatPanel({ className = '', onOpenLayerSidebar }: ChatPanelProps) {
  ...
  const handleLayerCompleted = useCallback(
    async (layer: number, reportContent: string, dimensionReports: Record<string, string>) => {
      ...
      if (backendData && backendData.reports && Object.keys(backendData.reports).length > 0) {
        ...
        // ✅ 详细日志：追踪每个维度的内容长度
        console.log(
          `[ChatPanel] Using REST API data: ${Object.keys(finalReports).length} dimensions, ${finalReportContent.length} chars`
        );
        ...
        const totalChars = Object.values(finalReports).reduce(
  ...
  useEffect(() => {
    ...
    layersToCheck.forEach(async ({ layer, completed }) => {
      ...
      if (messageExists) {
        ...
        // 消息存在但没有完整数据，需要获取数据
        console.log(
          `[ChatPanel] 🔄 SSE 断线恢复：Layer ${layer} 消息存在但数据不完整，补充数据...`
        );
      } else {
      ...
      try {
        ...
        if (backendData && backendData.reports && Object.keys(backendData.reports).length > 0) {
          console.log(
            `[ChatPanel] 🔄 SSE 断线恢复：获取到 Layer ${layer} 数据，${Object.keys(backendData.reports).length} 个维度`
          );
  ...
  const handleReviewApprove = useCallback(async () => {
    try {
      ...
      addMessage(
        createErrorMessage(`批准失败: ${error instanceof Error ? error.message : '未知错误'}`)
      );
    }
  }, [approve, addMessage]);
  ...
  const handleReviewReject = useCallback(
    async (feedback: string, dimensions?: string[]) => {
      try {
        ...
        addMessage(
          createErrorMessage(`驳回失败: ${error instanceof Error ? error.message : '未知错误'}`)
        );
      }
    },
    [reject, addMessage]
  ...
  const handleRollback = useCallback(
    async (checkpointId: string) => {
      ...
      try {
        ...
        addMessage(
          createErrorMessage(`回退失败: ${error instanceof Error ? error.message : '未知错误'}`)
        );
      } finally {
  ...
  const handleSendMessage = useCallback(async () => {
    ...
    if (isPaused && pendingReviewLayer) {
      ...
      try {
        ...
        addMessage(
          createErrorMessage(`修复失败: ${error instanceof Error ? error.message : '未知错误'}`)
        );
      }
      return;
    }
...
`,
  ...
  const _handleAction = useCallback(
    async (action: ActionButton, _message: Message) => {
      if (action.onClick) {
