...
def generate_spatial_layout_from_json(
    ...
    **kwargs
    ...
    2. Apply constraint zones (新增) - exclude protected areas
    3. Road-based boundary splitting
    4. Calculate parcel areas
    5. Allocate land use types by area_ratio and location_bias
    6. Merge adjacent same-type parcels
    7. Generate facility points
    8. Output GeoJSON
    ...
    Args:
        ...
        **kwargs: Additional parameters
            ...
            - constraint_zones: 约束区域字典 (新增) {
                "farmland_protection": GeoJSON,
                "ecological_protection": GeoJSON,
                "historical_protection": GeoJSON,
    ...
    try:
        ...
        # Step 2.1: Apply constraint zones (新增) - exclude protected areas
        constraint_zones = kwargs.get("constraint_zones", {})
        if constraint_zones:
            boundary_geom = _apply_constraint_zones(boundary_geom, constraint_zones)
            if boundary_geom is None or boundary_geom.is_empty:
                return {"success": False, "error": "No valid area after constraint exclusion"}
            # Recalculate area after constraint exclusion
            total_area_km2 = _calculate_area_km2(boundary_geom)

        # Step 2.2: Split boundary by roads or grid
        ...
        return {
            ...
            "data": {
                ...
                "statistics": {
                    ...
                    "facility_count": len(facilities_gdf) if facilities_gdf is not None else 0,
                    "axis_count": len(axes_gdf) if axes_gdf is not None else 0,
...
def _apply_constraint_zones(
    boundary_geom,
    constraint_zones: Dict[str, Dict[str, Any]]
) -> Any:
    """
    Apply constraint zones to boundary - exclude protected areas (新增).

    Args:
        boundary_geom: Original boundary geometry
        constraint_zones: 约束区域字典 {
            "farmland_protection": GeoJSON,
            "ecological_protection": GeoJSON,
            "historical_protection": GeoJSON,
            "hazard_zones": GeoJSON,
    ...
    Returns:
        Modified boundary geometry (excluding constraint areas)
    """
    from shapely.geometry import shape

    excluded_geoms = []

    for zone_type, zone_geojson in constraint_zones.items():
        if not zone_geojson or not zone_geojson.get("features"):
            continue

        try:
            zone_gdf = geojson_to_geodataframe(zone_geojson)
            if zone_gdf is not None and len(zone_gdf) > 0:
                # Merge all constraint geometries
                zone_union = zone_gdf.union_all()
                excluded_geoms.append(zone_union)
        except Exception as e:
            logger.warning(f"[constraint_zones] Failed to process {zone_type}: {e}")

    if not excluded_geoms:
        return boundary_geom

    # Combine all excluded areas
    all_excluded = unary_union(excluded_geoms)

    # Subtract excluded areas from boundary
    try:
        result_geom = boundary_geom.difference(all_excluded)
        logger.info(f"[constraint_zones] Excluded {len(excluded_geoms)} constraint zone types")
        return result_geom
    except Exception as e:
        logger.warning(f"[constraint_zones] Difference operation failed: {e}")
        return boundary_geom
...
def _split_boundary_by_roads(
    ...
    Enhanced with road extension and buffer for robust splitting.
    ...
    Args:
        ...
        use_buffer: Whether to apply small buffer to roads for better splitting
        buffer_meters: Buffer distance in meters (default 5m)
    ...
    if road_gdf is not None and len(road_gdf) > 0:
        try:
            # Get road lines and extend them to ensure full penetration
            road_lines = _prepare_cutting_lines(boundary_geom, road_gdf, use_buffer, buffer_meters)

            if road_lines:
                # Merge all road/cutting lines
                merged_roads = unary_union(road_lines)

                # Split boundary by roads
                split_result = split(boundary_geom, merged_roads)

                parcels = [g for g in split_result.geoms if g.is_valid and g.area > 0]
    ...
    if (not parcels or len(parcels) < 2) and fallback_grid:
...
def _prepare_cutting_lines(
    ...
    """
    Prepare cutting lines from road network.

    Extends roads to boundary edges and optionally applies buffer
    for more robust splitting.

    Args:
        boundary_geom: Boundary polygon
        road_gdf: Road network GeoDataFrame
        use_buffer: Whether to apply buffer
        buffer_meters: Buffer distance in meters

    Returns:
        List of prepared cutting line geometries
    """
    import geopandas as gpd
    ...
    buffer_deg = buffer_meters / meters_per_deg_lon if use_buffer else 0

    for road_geom in road_gdf.geometry:
        if road_geom is None or not boundary_geom.intersects(road_geom):
            continue
        ...
        # Extend road to boundary edges if needed
        extended_coords = _extend_line_to_boundary(coords, bounds)

        # Create extended line
        extended_line = geom.LineString(extended_coords)

        # Apply buffer if enabled
        if use_buffer and buffer_deg > 0:
            # Buffer creates polygon, extract centerline
            buffered = extended_line.buffer(buffer_deg)
            # Use the original extended line for splitting (buffer helps ensure penetration)
...
def _extend_line_to_boundary(
    ...
    """
    Extend line endpoints to boundary edges.

    This ensures the cutting line fully penetrates the boundary,
    enabling effective split operation.
    ...
    if abs(dx) > 1e-10 or abs(dy) > 1e-10:
        # Extend to boundary edge
        # Determine which edge the line should extend to
        if abs(dx) > abs(dy):
            # Horizontal-dominant line: extend to left/right edges
            # Start point
            if dx > 0:  # Line goes east
                # Extend start to west edge
                start_x = minx
            else:  # Line goes west
                # Extend start to east edge
                start_x = maxx
            # End point
            if dx > 0:
                end_x = maxx
            ...
            # Vertical-dominant line: extend to top/bottom edges
            if dy > 0:  # Line goes north
                start_y = miny
                end_y = maxy
            else:  # Line goes south
                start_y = maxy
                end_y = miny

    # Build extended coordinates
    extended = [(start_x, start_y)]
...
def _get_direction_from_center(
    ...
    # Use atan2 for direction calculation
    # atan2(dy, dx) returns:
    #   0° = east, 90° = north, ±180° = west, -90° = south
    angle = math.degrees(math.atan2(dy, dx))

    # Handle boundary cases with isclose to avoid floating-point issues
    # 45° and -45° are boundaries between east and north/south
    # 135° and -135° are boundaries between north/south and west
    if math.isclose(abs(angle), 45, abs_tol=0.1):
        # At 45° boundary - use quadrant-based decision
        if dx > 0 and dy > 0:
            return "north"  # Northeast -> prefer north
        elif dx > 0 and dy < 0:
            return "east"   # Southeast -> prefer east
        elif dx < 0 and dy > 0:
            return "north"  # Northwest -> prefer north
        else:
            return "south"  # Southwest -> prefer south
    elif math.isclose(abs(angle), 135, abs_tol=0.1):
        # At 135° boundary
        if dx < 0 and dy > 0:
            return "west"   # Northwest -> prefer west at this boundary
        else:
            return "south"  # Southwest -> prefer south at this boundary

    # Standard intervals (non-overlapping)
    if -45 < angle < 45:
        return "east"
    elif 45 < angle < 135:
        return "north"
