...
Architecture:
    API Layer → DimensionExecutor → analyze_dimension_simple
    ReviewService → DimensionExecutor → analyze_dimension_simple
"""
...
class DimensionExecutor:
    ...
    async def execute_dimensions(
        ...
        """
        from app.agent.nodes.dimension_node import (
            analyze_dimension_simple,
        ...
        async def run_one(dim_key: str) -> Tuple[str, str]:
            ...
            # 执行维度分析
            result = await analyze_dimension_simple(dim_state)
