...
class SectionBuilder:
            ...
            for sec_title, sec_content in sections.items():
                ...
                for pat in section_patterns:
                    ...
                    if pat in sec_title or sec_title in pat:
                        ...
                        matched_patterns.add(pat)
                        break
                    # 关键词匹配（处理同义词如'排水工程' vs '排水系统'）
                    if self._keyword_match(pat, sec_title):
                        matched = True
                        matched_patterns.add(pat)
                        break
                    # 模糊匹配 (降低阈值)
                    if self._fuzzy_match(pat, norm_title, threshold=0.65):
                        matched = True
                        matched_patterns.add(pat)
                        break
                    if self._fuzzy_match(pat, sec_title, threshold=0.65):
                        matched = True
                        matched_patterns.add(pat)
                        break
                if not matched:
    ...
    @staticmethod
    def _keyword_match(pat: str, title: str) -> bool:
        """关键词匹配：提取核心词进行匹配，如'排水工程' vs '排水系统'"""
        # 常见的同义词映射（后缀词）
        suffix_synonyms = {
            '工程': ['系统', '设施', '规划', '配置', '网络'],
            '系统': ['工程', '设施', '规划', '配置', '网络'],
        ...
        # 提取核心词（去掉常见的后缀词）
        suffixes = ['工程', '系统', '设施', '规划', '配置', '设计', '建设', '网络']
        pat_core = pat
        title_core = title
        for suffix in suffixes:
            if pat.endswith(suffix) and len(pat) > len(suffix):
                pat_core = pat[:-len(suffix)]
            if title.endswith(suffix) and len(title) > len(suffix):
                title_core = title[:-len(suffix)]

        # 核心词精确匹配
        if pat_core and title_core and pat_core == title_core:
            return True

        # 核心词包含匹配
        if pat_core and title_core and (pat_core in title_core or title_core in pat_core):
            return True

        # 核心词同义词匹配
        if pat_core in core_synonyms:
            for syn in core_synonyms[pat_core]:
                if syn == title_core or syn in title_core:
                    return True
        if title_core in core_synonyms:
            for syn in core_synonyms[title_core]:
                if syn == pat_core or syn in pat_core:
                    return True

        # 检查是否是后缀同义词变体
        for key, syns in suffix_synonyms.items():
            if pat.endswith(key) and any(title.endswith(s) for s in syns):
                pat_base = pat[:-len(key)]
                matched_syn = [s for s in syns if title.endswith(s)][0]
                title_base = title[:-len(matched_syn)]
                if pat_base == title_base:
                    return True
            if title.endswith(key) and any(pat.endswith(s) for s in syns):
                title_base = title[:-len(key)]
                matched_syn = [s for s in syns if pat.endswith(s)][0]
                pat_base = pat[:-len(matched_syn)]
                if pat_base == title_base:
                    return True
