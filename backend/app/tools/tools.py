...
使用 LangChain 原生 @tool 贅器定义所有工具，替代适配器层嵌套。

NOTE: 本模块定义的工具仅用于 LLM bind_tools (schema 层)。
实际执行通过 ToolRegistry 调用 GIS_TOOL_WRAPPERS 或 builtin tools。
以下工具仅存在于 schema 层，不通过 registry 执行:
- gis_analysis, network_analysis, population_prediction: 仅用于 schema 定义
- knowledge_search, web_search: 通过 builtin 模块执行
- accessibility_analysis: 通过 GIS_TOOL_WRAPPERS 执行
"""
