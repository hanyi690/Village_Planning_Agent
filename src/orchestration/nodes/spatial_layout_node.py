...
async def spatial_layout_node(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Step 3: Get GIS cache data
    gis_cache = state.get("gis_analysis_results", {})
    auto_fetched = gis_cache.get("_auto_fetched", {})

    # Extract boundary and road data
    boundary_geojson = None
    road_geojson = None

    if isinstance(auto_fetched, dict):
        boundary_data = auto_fetched.get("boundary")
        if boundary_data and isinstance(boundary_data, dict):
            boundary_geojson = boundary_data.get("geojson") or boundary_data

        road_data = auto_fetched.get("road")
        if road_data and isinstance(road_data, dict):
            road_geojson = road_data.get("geojson") or road_data
...
async def _parse_planning_text_to_scheme(
    ...
    # Create Flash LLM with JSON mode
