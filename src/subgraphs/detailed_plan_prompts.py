...
请基于当前时间确定规划编制日期和规划期限。规划期限应分为近期（1-2年）、中期（3-5年）、远期（5-10年）。
...
## 1. 产业定位与资源匹配
- **资源禀赋分析**：基于现状报告中的土地、气候、文化资源，分析产业发展基础
- **主导产业方向**：推导适合本村的1-2个主导产业
- **产业定位**：明确产业发展的总体方向和目标

## 2. 三链逻辑构建
- **强链——强化现有环节**：提升现有生产效率，优化生产流程
- **补链——补充缺失环节**：增加加工、仓储、冷链等配套环节
- **拓链——延伸产业链条**：延伸至电商、研学、文创、康养等新业态

## 3. 产业体系构建
- **第一产业**：种植、养殖基地规划（品种、规模、布局）
- **第二产业**：农产品加工、品牌设计（内容、规模、选址）
- **第三产业**：乡村旅游、电商服务、文创体验（业态、规模、布局）
- **产业融合模式**：一二三产融合发展的具体路径

## 4. 产业项目细化
每个产业项目包含：
- **项目名称**：简练明确
- **建设内容**：具体建设内容描述
- **建设规模**：种植面积、养殖规模、建筑面积等
- **选址位置**：具体位置描述
- **功能定位**：项目在产业链中的作用
...
4. 项目清单具体，建设规模量化
5. 实施路径明确，保障措施有力
6. 使用表格呈现产业项目清单（项目名称/建设内容/规模/选址）
...
|---------|-----|------|------|
| 给水管网 | 2km | 公里 | PE管 |
| 污水管网 | 1.5km | 公里 | DN300-600 |
| 污水处理站 | 50m³/d | 立方米/日 | 一体化设备 |
| 电力线路 | 3km | 公里 | 10kV/0.4kV |
| 路灯 | 60盏 | 盏 | LED |
...
## 4. 项目库汇总表
| 项目编号 | 项目名称 | 建设内容 | 规模 | 选址 | 分期 |
|---------|---------|---------|------|------|------|
| A01 | 村部改造 | 建筑改造、装修 | 200㎡ | 村中心 | 近期 |
| A02 | 文化中心 | 新建 | 300㎡ | 村中心 | 近期 |
| B01 | 道路硬化 | 水泥路面 | 2km | 主干道 | 近期 |
| B02 | 污水处理 | 处理站+管网 | 50m³/d | 村东 | 中期 |
...
3. 选址位置明确，布局合理
4. 实施时序合理，重点突出
5. 用表格汇总项目清单（项目名称/建设内容/规模/选址/分期）
...
**任务**：整合12个维度的专项研究，生成一份结构严谨、逻辑闭环的《村庄规划细化报告》。
...
2. **章节结构**：
   ...
   按【维度名称】分为12章，每章包含"现状洞察、规划思路、具体措施"三个模块：
...
3. **内容调性**：
   ...
   - **去资金化**：严禁出现任何具体的投资金额、产值数据或资金筹措比例


   - **量化工程**：措施必须具体到建设量（如：建设长度约XX米、设置XX处、占地XX公顷）


   - **术语专业**：使用如"三链模型"、"三区三线"、"适度集聚"、"弹性空间"等专业词汇
...
6. **表格呈现**：
   ...
   在土地利用、公共服务、建设项目库等章节必须使用 Markdown 表格列举项目与规模。
...
7. **完整结构示例**（产业规划）：
   ...
   强链——强化现有环节（统筹考虑村落现有产业的发展基础，选择最有发展潜力的产业作为抓手...）


   补链——围绕关键环节（在加大现有生产环节发展力度后，补全上游的研发环节...）


   拓链——发展关联产业（结合村落主导产业，拓宽产业链条，发挥村落剩余产能...）
